...
MapEditor::MapSnapshot MapEditor::CaptureSnapshot(const CampaignMap& map) {
    ...
    s.heightData = map.GetHeightMap().GetData(); return s;
}
...
void MapEditor::HandleLeftClick(const glm::vec3& wp, const glm::vec2& mp, CampaignMap& map) {
    ...
    if (currentTool == Tool::LINK_VERTEX) {
        auto h = FindNearestVertex(mp, map, 25.0f); auto oh = FindNearestObstacleVertex(mp, map, 25.0f);
        bool ip = (h.dist < oh.dist && h.provIdx >= 0); bool io = (!ip && oh.obsIdx >= 0);
        if (ip || io) {
            if (linkSourceProvIdx < 0 && linkSourceObsIdx < 0) {
                ...
                SaveUndoState(map);
                glm::vec3 src = (linkSourceProvIdx >= 0) ? map.GetProvinces()[linkSourceProvIdx].borderVertices[linkSourceVertIdx]
                ...
                if (ip) {
                    ...
                    glm::vec3 c(0); for (auto& v : ps[h.provIdx].borderVertices)c += v; ps[h.provIdx].center = c / (float)ps[h.provIdx].borderVertices.size();
                }
                else { map.GetObstaclesEditable()[oh.obsIdx].vertices[oh.vertIdx] = src; }
                geometryDirty = true; linkSourceProvIdx = -1; linkSourceVertIdx = -1; linkSourceObsIdx = -1; linkSourceObsVertIdx = -1;
            }
        } return;
    }
    if (currentTool == Tool::ADD_VERTEX && selectedProvinceIdx >= 0) {
        ...
        if (selectedProvinceIdx < (int)ps.size()) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                glm::vec2 ab = b - a; float l2 = glm::dot(ab, ab); if (l2 < 0.0001f)continue;
                float t = glm::clamp(glm::dot(ck - a, ab) / l2, 0.f, 1.f); float d = glm::distance(ck, a + t * ab);
                if (d < bd) { bd = d; be = i; }
            }
            int at = (be >= 0) ? be + 1 : n; p.borderVertices.insert(p.borderVertices.begin() + at, glm::vec3(wp.x, 0, wp.z));
            selectedVertexIdx = at; glm::vec3 c(0); for (auto& v : p.borderVertices)c += v; p.center = c / (float)p.borderVertices.size();
            geometryDirty = true;
        } return;
    }
    if (currentTool == Tool::HEIGHT_BRUSH) return;
    auto h = FindNearestVertex(mp, map, 18.0f); auto oh = FindNearestObstacleVertex(mp, map, 18.0f);
    if (h.dist < oh.dist && h.provIdx >= 0) { selectedProvinceIdx = h.provIdx; selectedVertexIdx = h.vertIdx; selectedObstacleIdx = -1; selectedObsVertexIdx = -1; }
    else if (oh.obsIdx >= 0) { selectedObstacleIdx = oh.obsIdx; selectedObsVertexIdx = oh.vertIdx; selectedProvinceIdx = -1; selectedVertexIdx = -1; }
    else {
        auto* pv = map.GetProvinceAtWorldPos(wp);
        if (pv) { for (int i = 0; i < (int)map.GetProvinces().size(); i++)if (map.GetProvinces()[i].id == pv->id) { selectedProvinceIdx = i; break; }selectedVertexIdx = -1; }
        else { selectedProvinceIdx = -1; selectedVertexIdx = -1; }selectedObstacleIdx = -1; selectedObsVertexIdx = -1;
    }
}
void MapEditor::HandleRightPress(const glm::vec3& wp, const glm::vec2& mp, CampaignMap& map) {
    if (!isActive)return;
    if (currentTool == Tool::MOVE_CITY) { int ci = FindNearestCity(mp, map, 22.0f); if (ci >= 0) { SaveUndoState(map); draggingCityProvIdx = ci; isDraggingCity = true; }return; }
    if ((selectedProvinceIdx >= 0 && selectedVertexIdx >= 0) || (selectedObstacleIdx >= 0 && selectedObsVertexIdx >= 0)) { SaveUndoState(map); isDragging = true; }
}
void MapEditor::HandleRightDrag(const glm::vec3& wp, CampaignMap& map) {
    if (!isActive)return; glm::vec3 np(wp.x, 0, wp.z);
    if (isDragging) {
        if (selectedProvinceIdx >= 0 && selectedVertexIdx >= 0) {
            auto& p = map.GetProvincesEditable()[selectedProvinceIdx];
            if (selectedVertexIdx < (int)p.borderVertices.size())MoveSharedVertices(map, p.borderVertices[selectedVertexIdx], np);
        }
        else if (selectedObstacleIdx >= 0 && selectedObsVertexIdx >= 0) {
            auto& ob = map.GetObstaclesEditable()[selectedObstacleIdx];
            if (selectedObsVertexIdx < (int)ob.vertices.size())MoveSharedVertices(map, ob.vertices[selectedObsVertexIdx], np);
        }
    }
    if (isDraggingCity && draggingCityProvIdx >= 0)map.GetProvincesEditable()[draggingCityProvIdx].cityPos = np;
}
void MapEditor::HandleRightRelease(CampaignMap& map) {
    if (isDragging) {
        if (selectedProvinceIdx >= 0) {
            auto& p = map.GetProvincesEditable()[selectedProvinceIdx];
            glm::vec3 c(0); for (auto& v : p.borderVertices)c += v; p.center = c / (float)p.borderVertices.size();
        }
        if (selectedObstacleIdx >= 0) {
            auto& ob = map.GetObstaclesEditable()[selectedObstacleIdx];
            glm::vec3 c(0); for (auto& v : ob.vertices)c += v; ob.center = c / (float)ob.vertices.size();
        }geometryDirty = true;
    }
    if (isDraggingCity)geometryDirty = true; isDragging = false; isDraggingCity = false;
}
void MapEditor::DeleteSelectedVertex(CampaignMap& map) {
    if (selectedProvinceIdx >= 0 && selectedVertexIdx >= 0) {
        ...
        if (p.borderVertices.size() > 3 && selectedVertexIdx < (int)p.borderVertices.size()) {
            SaveUndoState(map);
            p.borderVertices.erase(p.borderVertices.begin() + selectedVertexIdx); glm::vec3 c(0); for (auto& v : p.borderVertices)c += v;
            p.center = c / (float)p.borderVertices.size(); selectedVertexIdx = -1; geometryDirty = true;
        }return;
    }
    if (selectedObstacleIdx >= 0 && selectedObsVertexIdx >= 0) {
        ...
        if (ob.vertices.size() > 3 && selectedObsVertexIdx < (int)ob.vertices.size()) {
            SaveUndoState(map);
            ob.vertices.erase(ob.vertices.begin() + selectedObsVertexIdx); glm::vec3 c(0); for (auto& v : ob.vertices)c += v;
            ob.center = c / (float)ob.vertices.size(); selectedObsVertexIdx = -1; geometryDirty = true;
        }
    }
}
void MapEditor::HandleKeyPress(int key, bool ctrl, CampaignMap& map, Renderer& rend) {
    ...
    if (!ctrl) {
        ...
        if (key == GLFW_KEY_DELETE) { DeleteSelectedVertex(map); if (geometryDirty)RebuildGeometry(map, rend); }
        if (key == GLFW_KEY_R)RebuildGeometry(map, rend);
        if (currentTool == Tool::HEIGHT_BRUSH) {
            if (key == GLFW_KEY_Q)brushMode = BrushMode::RAISE;
            if (key == GLFW_KEY_G)brushMode = BrushMode::SMOOTH; if (key == GLFW_KEY_F)brushMode = BrushMode::FLATTEN;
        }
    }
    if (ctrl) {
        if (key == GLFW_KEY_Z)Undo(map, rend); if (key == GLFW_KEY_Y)Redo(map, rend);
        if (key == GLFW_KEY_S) { if (geometryDirty)RebuildGeometry(map, rend); MapSerializer::SaveToFile(map, "maps/europe_1700.json"); }
        if (key == GLFW_KEY_L) {
            if (MapSerializer::LoadFromFile(map, "maps/europe_1700.json")) {
                RebuildGeometry(map, rend);
                selectedProvinceIdx = -1; selectedVertexIdx = -1; selectedObstacleIdx = -1; selectedObsVertexIdx = -1; m_undoStack.clear(); m_redoStack.clear();
            }
        }
        if (key == GLFW_KEY_N) {
            SaveUndoState(map); std::vector<glm::vec3>vs = { {0,0,0},{1,0,0},{1,0,1},{0,0,1} };
            map.CreateProvince("New Province", "New City", "france", vs); selectedProvinceIdx = (int)map.GetProvinces().size() - 1;
            selectedVertexIdx = -1; geometryDirty = true; RebuildGeometry(map, rend);
        }
        if (key == GLFW_KEY_DELETE && selectedProvinceIdx >= 0) {
            SaveUndoState(map); map.DeleteProvince(selectedProvinceIdx);
            selectedProvinceIdx = -1; selectedVertexIdx = -1; geometryDirty = true; RebuildGeometry(map, rend);
        }
        if (key == GLFW_KEY_N) {
            // Create new province at camera center
            // User needs to have a position... use (0,0) as placeholder
            // They can move vertices after
            SaveUndoState(map);
            std::vector<glm::vec3> defaultVerts = {
                glm::vec3(0, 0, 0), glm::vec3(1, 0, 0),
                glm::vec3(1, 0, 1), glm::vec3(0, 0, 1)
            };
            map.CreateProvince("New Province", "New City", "france", defaultVerts);
            selectedProvinceIdx = (int)map.GetProvinces().size() - 1;
            selectedVertexIdx = -1;
            geometryDirty = true;
            RebuildGeometry(map, rend);
            Logger::Info("Editor: Created new province");
        }
        if (key == GLFW_KEY_DELETE && selectedProvinceIdx >= 0) {
            SaveUndoState(map);
            ...
            geometryDirty = true;
            RebuildGeometry(map, rend);
            Logger::Info("Editor: Deleted province");
        }
    }
...
void MapEditor::RebuildGeometry(CampaignMap& map, Renderer& rend) {
    ...
    rend.ClearMapGeometry(); rend.BuildMapGeometry(map); map.BuildNavGrid(); geometryDirty = false;
}
std::string MapEditor::GetToolName() const {
    switch (currentTool) {
    case Tool::SELECT:return"Select"; case Tool::ADD_VERTEX:return"Add Vertex"; case Tool::MOVE_CITY:return"Move City";
    case Tool::LINK_VERTEX:return(linkSourceProvIdx >= 0 || linkSourceObsIdx >= 0) ? "Link(target)" : "Link(source)";
    case Tool::HEIGHT_BRUSH: {
        char b[64]; const char* m = "Raise"; if (brushMode == BrushMode::SMOOTH)m = "Smooth"; if (brushMode == BrushMode::FLATTEN)m = "Flatten";
